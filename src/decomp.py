...
def filter_candidates (candidates):
    ...
    def candidate_sorting_key ((translation,(uses,message))):
    ...
    strongest_translation, (strongest_uses, strongest_message) = strongest
    deleted = 0
    for i, (_, (uses, _)) in enumerate (candidate_list[1:]):
...
for msgid, (comment, candidates) in new_messages.iteritems ():
    ...
    if len (candidate_list) > 1:
        ...
        for translation, (uses, message) in candidate_list:
            sys.stderr.write ((u" candidate: %s used %s times\n" % (translation, uses)).encode ("utf-8"))
    message = candidate_list[0][1][1]
